"""
MBSE+AI 自动化闭环系统 V3 — 主入口。

V3 新增：交叉校验 + 物理量验证 + 修复日志 + 追溯链 + stopTime适配。
...
LangGraph 状态图 + HITL interrupt + 质量自检 + 节点3子图自修复。
"""
...
def handle_interrupt(interrupt_data: dict) -> dict:
    ...
    if node == "node1":
        ...
        if qp.get("physics_passed") is not None:
            print(f"  物理预检: {'✅ 通过' if qp['physics_passed'] else '⚠️ 偏差 ' + str(qp.get('physics_deviation', '?')) + '%'}")
        print()
...
def main():
    ...
    # 创建输出目录
    run_dir = make_run_dir("outputs").resolve()
    ...
    initial_state: PipelineState = {
        ...
        "physics_feedback": "",
        # V4 新增
        "expected_physics": None,  # interactive 模式自动检测，experiment 模式从 test_case 读取
    }
