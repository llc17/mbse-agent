"""
=============================================================================
node2_sysml.py — Agent ② SysML Agent（V6 检索增强 + 三阶段审查版）
=============================================================================
定位: V6 质量提升最大的节点——SysML 代码是整个流水线的质量瓶颈。
      SysML 写错了，后面的 Modelica 一定错。
...
def _check_sysmlpy() -> tuple[bool, str]:
    ...
    try:
        ...
        _sysmlpy_version = ""
        logger.warning("sysmlpy 未安装，降级为 V3 正则语法检查")
    return _sysmlpy_available, _sysmlpy_version


def _build_parameter_replacements(params: dict) -> dict[str, str]:
    """从需求参数构建 prompt 占位符替换表。自动覆盖常见参数名。"""
    defaults = {
        "R": 1000.0, "R1": 1000.0, "R2": 10000.0, "C": 1e-6, "C1": 1e-6, "C2": 1e-7,
        "L": 1e-3, "L1": 1e-3, "V": 5.0, "V_in": 5.0, "Vcc": 15.0, "Vee": -15.0,
        "T": 298.15, "T_outdoor": 308.15, "T_indoor": 298.15,
        "Rf": 10000.0, "Rin": 1000.0, "R_f": 10000.0, "R_in": 1000.0, "Vin": 0.5,
    }
    return {f"{{parameters_{key}}}": str(params.get(key, val)) for key, val in defaults.items()}
...
def _generate(req: StructuredRequirement, sysml_dir: Path, temperature: float,
              feedback: str = "", max_syntax_retries: int = 3) -> SysMLArtifact:
    """
    V6 三阶段 SysML 生成: 检索示例 → 生成 → 审查 → 修正 → 语法检查。

    第 0 步: 检索官方示例（generate 和 review 各取一份，格式不同）
    第 1 步: 调用 LLM 生成 SysML 代码（prompt 中注入官方示例）
    第 2 步: 审查（对照官方示例 + 需求参数，逐条检查）
    第 3 步: 如有问题，针对性修正（只改问题，不动其他部分）
    第 4 步: 语法检查（第二层防线——正则 + sysmlpy）
    """
    ...
    def review_sysml(sysml_code: str) -> ReviewResult:
        """
        V6 结构化审查: 对照官方示例 + 需求参数逐条检查（7 条检查项）。

        审查用 chat_with("review", ...) → 环境变量 AGENT_REVIEW_PROVIDER=zhipu 启用智谱审查。
        智谱作为独立模型中立检查，避免 DeepSeek 自己审自己的盲区。
        """
        req_json = json.dumps({
    ...
    def revise_sysml(sysml_code: str, issues: list[ReviewIssue]) -> str:
        ...
        这是 V6 风险 4 的缓解方案（防止修正时不小心改坏对的）。
        """
        issues_text = _format_issues(issues)
        req_json_str = json.dumps({
            'component_type': req.component_type, 'parameters': req.parameters,
            'topology': req.topology, 'constraints': req.constraints,
        }, ensure_ascii=False, indent=2)
        ...
        revise_prompt = (
            ...
            f"## 官方参考示例\n{review_references}\n\n"
            f"## 要求\n1. 只修正列出的具体问题，不改其他正确部分\n"
            f"2. 修正后输出完整 SysML v2 代码\n3. 确保不引入新问题\n"
            f"4. 保持 package/import/part/port/connect 完整性\n"
            f"5. 只输出代码，不要解释"
        )
    ...
    loop_result = run_review_loop(
        generate_fn=generate_sysml, review_fn=review_sysml,
        revise_fn=revise_sysml, max_rounds=3, label="node2",
    )

    sysml_code = loop_result.final_output
    artifact = SysMLArtifact(sysml_code=sysml_code, attempts=loop_result.rounds)
    ...
    if syntax_errors:
        ...
        artifact.errors = [e for e in syntax_errors
...
# ============================================================================
# V4 保留: 语法检查（正则 + sysmlpy 标准解析器）
# ============================================================================

def _syntax_check(code: str) -> list[str]:
    """
    V4 语法检查: 正则快速检测 + sysmlpy 标准解析器（双层防线）。
    ...
    if sysmlpy_ok:
        ...
        try:
            ...
            errors.append(f"[error] sysmlpy: {str(e)[:200]}")
    return errors


def _syntax_fix(code: str, errors: list[str], req: StructuredRequirement,
                references: str) -> str:
    ...
    prompt = (
        ...
        f"## 当前代码\n```\n{code}\n```\n\n## 参考规范\n{references}\n\n"
        f"请修正以上语法错误，输出完整的 SysML v2 代码。只输出代码。"
    )
