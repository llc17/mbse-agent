"""
LangGraph 状态图 — V3 核心编排。

主图结构:
  START → node1_refine → node1_hitl → node2_generate → Q_cross_validate
       → node2_hitl → node3_subgraph → Q_physics_validate → node4_summary → END

V3 新增：
  - Q_cross_validate：需求 ↔ SysML 参数一致性校验，失败打回 node2
  - Q_physics_validate：仿真 CSV 物理量验证，失败打回 node3 repair
  - 修复日志：node3 子图每次 repair 记录 diff
  - 追溯链：prompt 嵌入来源标注
...
def build_pipeline() -> StateGraph:
    ...
    # V3: node3 → 物理验证 → node4（失败则打回 node3 repair）
